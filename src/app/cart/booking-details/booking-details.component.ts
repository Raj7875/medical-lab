...
export class BookingDetailsComponent implements OnInit {
   bookingForm !:FormGroup
        finalOrder:any
  constructor(private fb:FormBuilder,private cartservice:CartService,private http:HttpService,private router:Router) { 
    this.finalOrder = this.cartservice.getOrder();
  }
  ...
  bookOrder(){
   ...
   this.http.postDetailsToServer('orders',orderObj).subscribe((response)=>{
    if(response){
      // this.cartservice.removeProductsFromLocalStorage();
